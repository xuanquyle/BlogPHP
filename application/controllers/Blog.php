...
class Blog extends CI_Controller {
	...
	function submit_conmment(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$this->session->set_flashdata('msg','<div class="alert alert-info">Cảm ơn bạn đã phản hồi, bình luận của bạn sẽ xuất hiện sau khi kiểm duyệt</div>');
			redirect('blog/'.$slug);
		}
	}
	...
	function subscribe(){
		...
		$this->form_validation->set_rules('email', 'Email', 'required|valid_email');
		if ($this->form_validation->run() == FALSE){
			$this->session->set_flashdata('msg','<div class="alert alert-danger">Thông tin không hợp lệ!</div>');
			$base_url = site_url();
			redirect($base_url);
		}else{
			$email = $this->input->post('email',TRUE);
			...
			if($checking_email->num_rows() > 0){
				$this->session->set_flashdata('msg','<div class="alert alert-info">Cảm ơn bạn đã đăng ký.</div>');
				redirect($url);
			}else{
